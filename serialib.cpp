...
int serialib::getOutQueStat() {
    ...
    ClearCommError(hSerial, &commErrors, &commStatus);
    // Return the number of pending bytes
    return commStatus.cbOutQue;
#endif
#if defined(__linux__) || defined(__APPLE__)
    int nBytes = 0;
    // Return number of pending bytes in the sender queue
...
int serialib::getInQueMax() {
#if defined(_WIN32) || defined(_WIN64)
    COMMPROP prop;

    GetCommProperties(hSerial, &prop);

    return prop.dwMaxRxQueue;
#endif
#if defined(__linux__) || defined(__APPLE__)
    // Not implemented 
    return 0;
...
int serialib::getOutQueMax() {
#if defined(_WIN32) || defined(_WIN64)
    COMMPROP prop;

    GetCommProperties(hSerial, &prop);

    return prop.dwMaxTxQueue;
#endif
#if defined(__linux__) || defined(__APPLE__)
    // Not implemented 
    return 0;
